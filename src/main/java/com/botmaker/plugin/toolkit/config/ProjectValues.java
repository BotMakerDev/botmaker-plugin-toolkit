 ...
 * #1. What a bot's classpath does have is whatever its plugins bring, and the toolkit is what a plugin
 * compiles against at {@code compile} scope, so it arrives with any plugin the bot uses. The line this
 * module draws is the same one the contract draws: <i>the mechanism is the platform's, the vocabulary is
 * the plugin's</i>.
 *
...
public final class ProjectValues {
     ...
     *
     * <p><b>The report is unconditional.</b> It went through {@code shared}'s {@code Diag} while this class
     * lived there, which meant a bot run with debugging off said nothing and silently used defaults — the
     * worst available outcome, since the symptom is then "the default, always" with no line to explain it.
     * The toolkit cannot see {@code Diag} and must not grow a second switch beside it, and these messages
     * are once per process and describe a broken project rather than a trace. So they print.
     */
    public static ProjectValues load(String resource) {
        try (InputStream in = ProjectValues.class.getResourceAsStream(resource)) {
            ...
            System.err.println("[config] " + resource + " could not be read (" + e.getMessage()
    ...
    public static ProjectValues of(String json) {
        ...
        try {
            ...
            System.err.println("[config] model could not be parsed (" + e.getMessage() + ")");
            return EMPTY;
        }
    }
