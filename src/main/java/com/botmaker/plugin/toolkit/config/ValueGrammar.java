 ...
 *
 * <p><b>There is no grammar in the toolkit itself, and that is the point.</b> This module owns the
 * mechanism; a vocabulary belongs to whoever introduced it. Shipping "the obvious JDK ones" here would put a
 * second {@code Duration} parser beside the SDK's and start exactly the drift the design exists to prevent —
 * and it would make this module a vocabulary, which is the one thing the platform's own test refuses. The
 * JDK types are the SDK's today only because the SDK is the only plugin; they belong in a plugin of their
 * own, and this interface is what such a plugin would be written against.
 *
 * <h2>Totality</h2>
 *
